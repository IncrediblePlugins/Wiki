...
will accumulate each limit. Example: A player has lands.lands.5 and lands.lands.4 set. This would result in lands.lands.9.
...
to lands he's member of? Example: lands.chunks.support.5 -> each land in which the player is trusted and not owned by the player, will receive an additional amount of 5 claims.
...
Description: Set the max. amount of disconnected parts for the players owned lands. This includes the initial claim. So setting `lands.parts.1`, won't do anything.
This will only take effect, if the option `treat-as-new` is disabled in config.yml. This permission allows players to claim a collections of chunks that are disconnected from the original land, but the `force-near` option will still be enforced once the maximum amount is reached. 
...
## Command Permissions
Commands wiki page: https://lands.incredibleplugins.com/wiki/Commands

## Teleportation
The following permissions limit all teleportation initiated by Lands. Players have them by default. However, in some cases
