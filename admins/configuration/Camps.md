...
A camp does not pay any upkeep. You can give camp items to players by executing `/lands admin player <player> give camp <radius> <amount> [silent]`.

The configuration can be found in config.yml:
````yaml
# Camps are temporary lands that will get deleted after a defined period.
# Camp items can be given at first join or by executing /lands admin player <player> give camp <radius> <amount> [silent]
camp:
