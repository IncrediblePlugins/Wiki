...
Create a nation with `/nations create <capital>`, where `<capital>` is the land that should become
the capital. You must own that land, and it cannot already be in a nation.
...
On some servers, lands in a nation pay upkeep to the nation instead of directly to the server.
...
If you are trusted in lands on both sides of the same war, your team is chosen once for that war. Defending lands are prioritized.

# Nation Levels

Nations can level up if your server has nation levels enabled.
